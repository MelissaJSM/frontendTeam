document.addEventListener("DOMContentLoaded", () => {
  ...
  let row = "<tr><th colspan='2'>도서 정보</th><th>도서명</th><th>저자</th><th>수량</th><th>도서가격</th></tr>";
  let order_chk = document.getElementById("order_chk");
  let total = 0;
  ...
  for(let i = 0; i < jsonData.length; i++){
    row += `<tr><td><img src=${jsonData[i].book_src}></td>
								...
								<td>${jsonData[i].book_quantity}</td>
								<td class ='price_chk'>${jsonData[i].book_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td></tr>`;
    total += (jsonData[i].book_quantity * jsonData[i].book_price);
  };

  row +=`<tr id="total"><td colspan='2'>총 결제 금액</td><td colspan='4'>${total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} 원<td></tr>`;
  order_chk.innerHTML = row;
});
